...
float diffraction(vec2 world_pos) {
    float diffraction_index = ((noise((world_pos * 0.5) / (sprite_size / 2048)))) * 0.01; 
...
void handle_glass(vec4 pixel) {
    ...
    vec2 diffracted_pos = screen_pos + vec2(diffraction(world_pos));
    vec4 pixel_world = texture(world, diffracted_pos);


    // fragColor.rgb = vec3(diffraction(world_pos)) * 100;
